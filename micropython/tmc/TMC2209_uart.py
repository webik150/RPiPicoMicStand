...
class TMC_UART:
    ...
    def __init__(self, serialport, baudrate, rxpin, txpin, mtr_id_arg):
        self.ser = UART(serialport, baudrate, bits=8, parity=None, stop=1, tx=txpin, rx=rxpin)
    ...
    def read_reg(self, reg):
        ...
        if rt != len(self.rFrame):
            ...
            return False
        utime.sleep(self.communication_pause)  # adjust per baud and hardware. Sequential reads without some delay fail.
        if self.ser.any():
        ...
        # print list of int as hex values
        #print("wrote: "+str([hex(x) for x in self.rFrame]))
        #print("received: "+str(binascii.hexlify(rtn)))
        return (rtn[7:11])
    ...
    def write_reg(self, reg, val):
        ...
        if rtn != len(self.wFrame):
            print("TMC2209: Err in write {}".format(__), file=sys.stderr)
            return False
        utime.sleep(self.communication_pause)
